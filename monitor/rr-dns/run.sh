...
curl -X GET "https://api.cloudflare.com/client/v4/zones/$CLOUDFLARE_ZONE_ID/dns_records?type=A&name=test1.notional.ventures&match=all" -H "Content-Type:application/json" -H "X-Auth-Email:$CLOUDFLARE_X_AUTH_EMAIL" -H "X-Auth-Key:$CLOUDFLARE_X_AUTH_KEY" > $HOME/dns_records.txt
