...
eval "$(curl -s https://raw.githubusercontent.com/notional-labs/cosmosia/104-add-kujira/data/chain_registry.ini |awk -v TARGET=$chain_name -F ' = ' '
  {
    if ($0 ~ /^\[.*\]$/) {
      gsub(/^\[|\]$/, "", $0)
      SECTION=$0
...
curl -Ls "https://raw.githubusercontent.com/notional-labs/cosmosia/104-add-kujira/snapshot/snapshot.nginx.conf" > /etc/nginx/nginx.conf
# mkdir -p /snapshot
/usr/sbin/nginx
...
curl -Ls "https://raw.githubusercontent.com/notional-labs/cosmosia/104-add-kujira/rpc/start_chain.sh" > $HOME/start_chain.sh
curl -Ls "https://raw.githubusercontent.com/notional-labs/cosmosia/104-add-kujira/snapshot/snapshot_download.sh" > $HOME/snapshot_download.sh
...
curl -Ls "https://raw.githubusercontent.com/notional-labs/cosmosia/104-add-kujira/snapshot/scripts/pebblecompact_data.sh" > $HOME/pebblecompact_data.sh
...
curl -Ls "https://raw.githubusercontent.com/notional-labs/cosmosia/104-add-kujira/snapshot/snapshot_cronjob.sh" > $HOME/snapshot_cronjob.sh
